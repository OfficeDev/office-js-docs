...
|[getSpillParent()](#getspillparent)|[Range](range.md)|Gets the range object containing the anchor cell for a cell getting spilled into. Fails if applied to a range with more than one cell. Read only.|[beta](../requirement-sets/excel-api-requirement-sets.md)|
|[getSpillingToRange()](#getspillingtorange)|[Range](range.md)|Gets the range object containing the spill range when called on an anchor cell. Fails if applied to a range with more than one cell. Read only.|[beta](../requirement-sets/excel-api-requirement-sets.md)|
